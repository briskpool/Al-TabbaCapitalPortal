                    ...
                    <tr>
                        <td style="text-align:center;">
                            <a href="{{config('email_template.website_link')}}" title="logo" target="_blank">
                                <img height="50" src="{{config('email_template.logo')}}"
                                    title="logo" alt="logo">
                            </a>
                        </td>
                    </tr>
                                    ...
                                    <td style="padding:0 35px;">
                                        ...
                                        @if ($transaction_data['transaction_type'] == 'deposit')
                                            <p
                                                ...
                                                Your payment of <strong>{{$transaction_data['currency_symbol']." ". number_format($transaction_data['transaction_amount'], 2) }}</strong> has been confirmed with {{ env('APP_NAME') }}.  It has now been added to your <strong>{{$transaction_data['membership_type']}}</strong> account with {{ env('APP_NAME') }}.
                                                If you have any questions, we're always happy to help out, just email us.
                                            </p>
                                        @else
                                            <p
                                                style="color:#455056;line-height:24px; margin:20px auto; font-weight: 500; text-align:center">
                                                Your withdrawal of <strong>{{$transaction_data['currency_symbol']." ". number_format($transaction_data['transaction_amount'], 2) }}</strong> from <strong>{{$transaction_data['membership_type']}}</strong> has been initiated with {{ env('APP_NAME') }}. It will now be passed to your Account Manager {{env('ACCOUNT_MANAGER')}}.
                                                If you have any questions, we're always happy to help out, just email us.
                                            </p>
                                        @endif
                                        <p
